...
func TestConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid config",
			config:  Config{CriticalAgeDays: 12, MaxAgeDays: 30},
			wantErr: false,
		},
		{
			name:    "critical >= max",
			config:  Config{CriticalAgeDays: 30, MaxAgeDays: 30},
			wantErr: true,
		},
		{
			name:    "negative critical",
			config:  Config{CriticalAgeDays: -1, MaxAgeDays: 30},
			wantErr: true,
		},
		{
			name:    "version-based policy (both zero)",
			config:  Config{CriticalAgeDays: 0, MaxAgeDays: 0},
			wantErr: false,
		},
	}
