...
2. Support both **time-based** expiry policies (days) and **version-based** policies (N minor versions)
3. Maintain embedded caches for common projects
4. Allow custom cache files for other projects
5. Default to current behavior (actions/runner with 30-day policy) for backward compatibility
6. Keep the current tool name: `github-actions-runner-version`
...
2. Create `internal/policy` package with policy interface
3. Create `internal/cache` package with cache manager
4. Modify `internal/github/client.go` to accept owner/repo parameters
5. Update `internal/version/checker.go` to use policy system
6. Add tests for all new packages
...
2. Implement repository name resolution (aliases)
3. Implement URL parsing (extract owner/repo from GitHub URLs)
4. Maintain backward compatibility (default to actions/runner)
...
2. Create cache JSON files in `data/` directory
3. Update `cmd/bootstrap-releases` to accept `--repo` flag
4. Update `cmd/check-releases` to accept `--repo` flag
5. Create `scripts/bootstrap-cache.sh` for new repositories
6. Update `.github/workflows/update-releases.yml` to handle multiple repos
...
2. End-to-end tests for custom cache files
...
6. Add examples for each supported repository
...
2. **Existing flags preserved**: All current flags continue to work
3. **Binary name unchanged**: Still `github-actions-runner-version`
4. **Cache path migration**: `data/releases.json` → `data/actions-runner.json` (symlink for compatibility)
...
### Medium Risk
- ⚠️  Cache file size growth (4 embedded caches instead of 1)
  - **Mitigation**: Limit cache depth to last 100 releases per repo
- ⚠️  Version-based policy complexity
...
2. ✅ Both time-based and version-based policies working
3. ✅ Custom cache files supported
4. ✅ Zero breaking changes for existing users
5. ✅ Test coverage remains above 45%
6. ✅ Binary size remains under 12MB
