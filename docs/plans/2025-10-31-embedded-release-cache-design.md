...
1. `GetAllReleases()` - fetch up to 1000 releases (10 pages × 100 per page)
...
1. **Maintain accuracy** - Stay current with recent releases
1. **Preserve atomicity** - CLI remains single portable binary
1. **Automatic updates** - Tool rebuilds when new runner releases appear
1. **Rate limit friendly** - Predictable, minimal API usage
...
1. API call: Fetch 5 most recent releases (single request)
1. Validate: Is embedded latest release in those 5?
...
1. Create `scripts/update-releases.sh`
1. Create `cmd/bootstrap-releases/main.go`
1. Run script to generate initial `data/releases.json`
1. Commit to repo

### Phase 2: Embedded Data Loading

1. Create `internal/data/loader.go` with `//go:embed`
1. Add `LoadEmbeddedReleases()` function
1. Test loading and parsing

### Phase 3: Update Runtime Logic

1. Add `GetRecentReleases(count int)` to GitHub client
1. Update `Checker.Analyse()` to use embedded + API strategy
1. Add `isEmbeddedCurrent()` and `mergeReleases()` helpers
1. Test with mock data

### Phase 4: Automated Updates

1. Create `cmd/check-releases/main.go`
1. Create `.github/workflows/update-releases.yml`
1. Test workflow (manual dispatch)
1. Enable daily cron

### Phase 5: Testing & Validation

1. Unit tests for merge logic
1. Integration tests for cache validation
1. Manual testing with real API
1. Verify binary size impact
...
1. Differential updates (only fetch releases after certain date)
1. Multiple cache tiers (embedded + user-local cache file)
1. Metrics on cache hit/miss rates
