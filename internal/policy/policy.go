...
func NewPolicy(repoConfig *config.RepositoryConfig) VersionPolicy {
	switch repoConfig.PolicyType {
	case config.PolicyTypeDays:
		return &DaysPolicy{
			CriticalDays: repoConfig.CriticalDays,
			MaxDays:      repoConfig.MaxDays,
		}
	case config.PolicyTypeVersions:
		return &VersionsPolicy{
			MaxMinorVersionsBehind: repoConfig.MaxVersionsBehind,
		}
	default:
